<nav class="main-header navbar navbar-expand navbar-light">
    ...
    <div class="ml-auto">
        <h5 class="text-center text-bold">{{\Modules\Setting\Entities\Setting::where('setting_key','core.company_name')->first()->setting_value}}</h5>
    </div>
        ...
        <li class="nav-item dropdown user-menu">
            <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
                @if(!empty(Auth::user()->photo))
                    <img class="user-image img-circle"
                         src="{{asset('storage/uploads/user/'.Auth::user()->photo)}}"
                         alt="User Image">
                @else
                    <img class="user-image img-circle"
            ...
            <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                <!-- User image -->
                <li class="user-header bg-brown-dark">
                    @if(!empty(Auth::user()->photo))
                        <img class="user-image img-circle"
                             src="{{asset('storage/uploads/user/'.Auth::user()->photo)}}"
                             alt="User Image">
