...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = User::when($orderBy, function (Builder $query) use ($orderBy, $orderByDir) {
            ...
            ->when(true, function (Builder $query) {
                $query->where('state_admin', '=', 0);
            })
            ->paginate($perPage)
            ->appends($request->input());
           // dd($data);
        foreach ($data as $key => $user) {
            $roles = Role::leftJoin('model_has_roles', 'model_has_roles.role_id', 'roles.id')->where('model_has_roles.model_id', $user->id)->get();
            $data[$key]->role_details = "";
            foreach ($roles as $role) {
                if ($data[$key]->role_details != "") {
                    $data[$key]->role_details .= ",";
                }
                $data[$key]->role_details .= $role->name;
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        $roleIds = explode(',', $request->input('roles.0'));

        // Retrieve role models based on the IDs
        $roles = Role::whereIn('id', $roleIds)->get();

        // Extract role names from the role models
