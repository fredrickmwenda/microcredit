...
@section('title')
    {{ trans_choice('share::general.share',2) }}
@endsection
@section('styles')
@stop
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('share::general.share',2) }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('share::general.share',2) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="card">
            <div class="card-header">
                @can('share.shares.create')
                    <a href="{{ url('share/create') }}" class="btn btn-info btn-sm">
                        <i class="fas fa-plus"></i> {{ trans_choice('core::general.add',1) }} {{ trans_choice('share::general.share',1) }}
                    </a>
                @endcan
                <div class="btn-group">
                    <div class="dropdown">
                ...
                <div class="card-tools">
                    <form class="form-inline ml-0 ml-md-3" action="{{url('share')}}">
            ...
            <div class="card-body p-0">
                <table id="data-table" class="table table-striped table-condensed table-hover">
                    ...
                    <tr>
                        ...
                        <th>
                            <a href="{{table_order_link('savings_id')}}">
                                <span>{{ trans_choice('savings::general.savings',1) }} {{ trans_choice('share::general.account',1) }}</span>
                            </a>
                        </th>
                        <th>
                            <a href="{{table_order_link('approved_shares')}}">
                                <span>{{ trans_choice('share::general.approved',1) }} {{ trans_choice('share::general.share',2) }}</span>
                            </a>
                        </th>
                        <th>
                            <a href="{{table_order_link('status')}}">
                                <span>{{ trans_choice('share::general.status',1) }}</span>
                            </a>
                        </th>
                        <th>
                            <a href="{{table_order_link('share_product')}}">
                                <span>{{ trans_choice('share::general.product',1) }}</span>
                            </a>
                        </th>
                    ...
                    @foreach($data as $key)
                        <tr>
                            ...
                            <td>
                                <a href="{{url('savings/'.$key->savings_id.'/show')}}">
                                    <span>{{$key->savings_id}}</span>
                                </a>
                            </td>
                            <td>
                                <span>{{number_format($key->total_shares,2)}}</span>
                            </td>
                            <td>
                                ...
                                @if($key->status == 'overpaid')
                                    <span class="badge badge-warning">{{trans_choice('share::general.overpaid', 1)}}</span>
                                @endif
                                @if($key->status == 'approved')
                                    <span class="badge badge-warning">{{trans_choice('share::general.awaiting_disbursement', 1)}}</span>
                                @endif
                                @if($key->status == 'active')
                                    <span class="badge badge-info">{{trans_choice('share::general.active', 1)}}</span>
                                @endif
                                ...
                                @if($key->status == 'closed')
                                    <span class="badge badge-success">{{trans_choice('share::general.closed', 1)}}</span>
                                @endif
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{url('share/' . $key->id . '/show')}}" class="dropdown-item">
