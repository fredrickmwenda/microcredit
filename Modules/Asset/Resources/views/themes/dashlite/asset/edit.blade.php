...
@section('title')
    {{ trans_choice('core::general.edit',1) }} {{ trans_choice('asset::general.asset',1) }}
@endsection
@section('content')
        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title"> {{ trans_choice('core::general.edit',1) }} {{ trans_choice('asset::general.asset',1) }}</h3>
    ...
    <div class="nk-block nk-block-lg" id="app">
        <form method="post" action="{{ url('asset/'.$asset->id.'/update') }}">
            {{csrf_field()}}
                    ...
                    <div class="row gy-4">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="branch_id"
                                       class="control-label">{{trans_choice('core::general.branch',1)}}/ Team {{trans_choice('accounting::general.account',1)}}
                                </label>
    ...
    <script>
        var app = new Vue({
            ...
            data: {
                ...
                asset_type_id: parseInt("{{old('asset_type_id',$asset->asset_type_id)}}"),
                branch_id: parseInt("{{old('branch_id',$asset->branch_id)}}"),
                purchase_date: "{{old('purchase_date',$asset->purchase_date)}}",
                purchase_price: "{{old('purchase_price',$asset->purchase_price)}}",
                replacement_value: "{{old('replacement_value',$asset->replacement_value)}}",
                value: "{{old('value',$asset->value)}}",
                life_span: "{{old('life_span',$asset->life_span)}}",
                salvage_value: "{{old('salvage_value',$asset->salvage_value)}}",
                serial_number: "{{old('serial_number',$asset->serial_number)}}",
                bought_from: "{{old('bought_from',$asset->bought_from)}}",
                purchase_year: "{{old('purchase_year',$asset->purchase_year)}}",
                status: "{{old('status',$asset->status)}}",
                active: "{{old('active',$asset->active)}}",
                notes: `{{old('notes',$asset->notes)}}`,
