...
@section('title')
    {{ trans_choice('income::general.income',2) }}
@endsection
@section('styles')
@stop
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('income::general.income',1) }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('income::general.income',1) }} </li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="card">
            <div class="card-header">
                    @can('income.income.create')
                        <a href="{{ url('income/create') }}" class="btn btn-info btn-sm">
                            <i class="fas fa-plus"></i> {{ trans_choice('core::general.add',1) }} {{ trans_choice('income::general.income',1) }}
                        </a>
                    @endcan
                <div class="btn-group">
                    <div class="dropdown">
                ...
                <div class="card-tools">
                    <form class="form-inline ml-0 ml-md-3" action="{{url('income')}}">
            ...
            <div class="card-body p-0">
                <table id="data-table" class="table table-striped table-condensed table-hover">
                    ...
                    <tr>
                        ...
                        <th>
                            <a href="{{table_order_link('income_type')}}">
                                <span>{{trans_choice('income::general.income',1)}} {{trans_choice('core::general.type',1)}}</span>
                            </a>
                        </th>
                        <th>
                            <a href="{{table_order_link('amount')}}">
                                <span>{{trans_choice('income::general.amount',1)}}</span>
                            </a>
                        </th>
                        <th>
                    ...
                    @foreach($data as $key)
                        <tr>
                            <td>
                                <a href="{{url('income/' . $key->id . '/show')}}">
                                    <span>{{$key->id}}</span>
                                </a>
                            </td>
                            <td>
                                <span>{{$key->income_type}}</span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="{{url('income/' . $key->id . '/show')}}"  class="dropdown-item">
                                            <i class="ri-eye-fill"></i>
                                            <span>{{trans_choice('core::general.detail',2)}}</span>
                                        </a>
                                        @can('income.income.edit')
                                            <a href="{{url('income/' . $key->id . '/edit')}}"
                                               class="dropdown-item">
                                        ...
                                        @can('income.income.destroy')
                                            <a href="{{url('income/' . $key->id . '/destroy')}}"
