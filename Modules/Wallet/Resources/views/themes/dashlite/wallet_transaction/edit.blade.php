...
@section('title')
    {{ trans_choice('core::general.edit',1) }} {{ trans_choice('wallet::general.transaction',1) }}
@endsection
@section('content')
        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">{{ trans_choice('core::general.edit',1) }} {{ trans_choice('wallet::general.transaction',1) }}</h3>
    ...
    <div class="nk-block nk-block-lg" id="app">
        <form method="post" action="{{url('wallet/transaction/'.$wallet_transaction->id.'/update')}}">
            {{csrf_field()}}
...
@section('scripts')
    <script>
        ...
        var app = new Vue({
            el: '#app',
            data: {
                amount: "{{old('amount',$wallet_transaction->amount)}}",
                date: "{{old('date',$wallet_transaction->submitted_on)}}",
                payment_type_id: parseInt("{{old('payment_type_id',$wallet_transaction->payment_detail->payment_type_id)}}"),
                account_number: "{{old('account_number',$wallet_transaction->payment_detail->account_number)}}",
                cheque_number: "{{old('cheque_number',$wallet_transaction->payment_detail->cheque_number)}}",
                routing_code: "{{old('routing_code',$wallet_transaction->payment_detail->routing_code)}}",
                receipt: "{{old('receipt',$wallet_transaction->payment_detail->receipt)}}",
                bank_name: "{{old('bank_name',$wallet_transaction->payment_detail->bank_name)}}",
                description: `{{old('description',$wallet_transaction->payment_detail->description)}}`,
