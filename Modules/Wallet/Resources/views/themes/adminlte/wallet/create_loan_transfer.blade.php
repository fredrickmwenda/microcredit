...
@section('title')
    {{ trans_choice('core::general.add',1) }} {{ trans_choice('wallet::general.loan_transfer',1) }}
@endsection
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a
                                    href="{{url('wallet/'.$wallet->wallet_id.'/show')}}">{{ trans_choice('wallet::general.wallet',2) }}</a>
                        </li>
                        <li class="breadcrumb-item active">{{ trans_choice('core::general.add',1) }} {{ trans_choice('wallet::general.loan_transfer',1) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content" id="app">
        <form method="post" action="{{url('wallet/'.$wallet->id.'/transfer/loan/store')}}">
            {{csrf_field()}}
            <div class="card card-bordered card-preview">
                <div class="card-body">
                    <div class="form-group">
                        <label class="control-label"
                               for="loan_id">{{trans_choice('loan::general.loan',1)}}</label>
                        <select class="form-control" name="loan_id" id="loan_id" v-model="loan_id" required>
                            <option value="" disabled selected>{{trans_choice('core::general.select',1)}}</option>
                            @foreach($wallet->client->loans as $key)
                                <option value="{{$key->id}}">{{trans_choice('loan::general.loan',1)}}
                                    #{{$key->id}}</option>
                            @endforeach
