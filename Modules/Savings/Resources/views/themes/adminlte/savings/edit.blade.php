...
@section('title')
    {{ trans_choice('core::general.edit',1) }} {{ trans_choice('savings::general.savings',1) }}
@endsection
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('core::general.edit',1) }} {{ trans_choice('savings::general.savings',1) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content" id="app">
        <form method="post" action="{{ url('savings/'.$savings->id.'/update') }}">
            {{csrf_field()}}
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="account_number"
                                       class="control-label">{{trans_choice('savings::general.account_number',1)}}</label>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="external_id"
                                       class="control-label">{{trans_choice('savings::general.external_id',1)}}</label>
...
@section('scripts')
    <script>
        ...
        var app = new Vue({
            el: '#app',
            data: {
                client_id: parseInt("{{old('client_id',$savings->client_id)}}"),
                savings_officer_id: parseInt("{{old('savings_officer_id',$savings->savings_officer_id)}}"),
                savings_product_id: parseInt("{{old('savings_product_id',$savings->savings_product_id)}}"),
                account_number: "{{old('account_number',$savings->account_number)}}",
                external_id: "{{old('external_id',$savings->external_id)}}",
                interest_rate: "{{old('interest_rate',$savings->interest_rate)}}",
                lockin_period: "{{old('lockin_period',$savings->lockin_period)}}",
                lockin_type: "{{old('lockin_type',$savings->lockin_type)}}",
                automatic_opening_balance: "{{old('automatic_opening_balance',$savings->automatic_opening_balance)}}",
                submitted_on_date: "{{old('submitted_on_date',$savings->submitted_on_date)}}",
                overdraft_limit: "{{old('overdraft_limit',$savings->overdraft_limit)}}",
                overdraft_interest_rate: "{{old('overdraft_interest_rate',$savings->overdraft_interest_rate)}}",
                minimum_overdraft_for_interest: "{{old('minimum_overdraft_for_interest',$savings->minimum_overdraft_for_interest)}}",
                savings_products: savings_products,
                savings_product_charges: {!! json_encode($savings->savings_product->charges) !!},
                savings_charges: savings_charges,
                clients: clients,
                users: users,
                selected_charge: '',
                savings_product: {!! json_encode($savings->savings_product) !!},
