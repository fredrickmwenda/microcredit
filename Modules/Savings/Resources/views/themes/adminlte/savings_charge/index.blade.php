...
@section('title')
    {{ trans_choice('savings::general.savings',1) }}  {{ trans_choice('savings::general.charge',2) }}
@endsection
@section('styles')
@stop
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{ trans_choice('savings::general.savings',1) }}  {{ trans_choice('savings::general.charge',2) }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ trans_choice('savings::general.savings',1) }}  {{ trans_choice('savings::general.charge',2) }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
        ...
        <div class="card">
            <div class="card-header">
                @can('savings.savings.charges.create')
                    <a href="{{ url('savings/charge/create') }}" class="btn btn-info btn-sm">
                        <i class="fas fa-plus"></i> {{ trans_choice('core::general.add',1) }} {{ trans_choice('savings::general.charge',1) }}
                    </a>
                @endcan
                <div class="btn-group">
                    <div class="dropdown">
                ...
                <div class="card-tools">
                    <form class="form-inline ml-0 ml-md-3" action="{{url('savings/charge')}}">
            ...
            <div class="card-body p-0">
                <table id="data-table" class="table table-striped table-condensed table-hover">
                    ...
                    <tr>
                        ...
                        <th>
                            <a href="{{table_order_link('savings_charge_type_id')}}">
                                <span>{{ trans_choice('savings::general.charge',1) }} {{ trans_choice('core::general.type',1) }}</span>
                            </a>
                        </th>
                        <th>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($key->savings_charge_type_id==1)
                                    <span>{{trans_choice('savings::general.savings_activation', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==2)
                                    <span>{{trans_choice('savings::general.specified_due_date', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==3)
                                    <span>{{trans_choice('savings::general.withdrawal_fee', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==4)
                                    <span>{{trans_choice('savings::general.annual_fee', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==5)
                                    <span>{{trans_choice('savings::general.monthly_fee', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==6)
                                    <span>{{trans_choice('savings::general.inactivity_fee', 1)}}</span>
                                @endif
                                @if($key->savings_charge_type_id==7)
                                    <span>{{trans_choice('savings::general.quarterly_fee', 1)}}</span>
                                @endif
                            </td>
                            <td>
                                @if($key->savings_charge_option_id==1)
                                    <span>{{number_format($key->amount, 2)}} {{trans_choice('savings::general.flat', 1)}}</span>
                                @endif
                                @if($key->savings_charge_option_id==2)
                                    <span>{{number_format($key->amount, 2)}} % {{trans_choice('savings::general.percentage_of_amount', 1)}}</span>
                                @endif
                                @if($key->savings_charge_option_id==3)
                                    <span>{{number_format($key->amount, 2)}} % {{trans_choice('savings::general.percentage_of_savings_balance', 1)}}</span>
                                @endif
                            </td>
                            <td>
                                @if($key->active==1)
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        ...
                                        @can('savings.savings.charges.destroy')
                                            ...
                                            <a href="{{url('savings/charge/' . $key->id . '/destroy')}}"
