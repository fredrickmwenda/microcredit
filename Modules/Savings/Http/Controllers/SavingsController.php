...
//        }
    ...
    public function store_deposit(Request $request, $id)
    {
        ...
        event(new TransactionUpdated($savings));
        $previous_status = "";
        event(new SavingsStatusChanged($savings, $previous_status, $savings_transaction->id));
        \flash(trans_choice("core::general.successfully_saved", 1))->success()->important();
        return redirect('savings/' . $savings->id . '/show');
    }

    public function store_deposit2(Request $request)
    {
    ...
    public function getSavingsDepositReport(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($query)->editColumn('id', function ($data) {
                ...
                if ($data->status == "approved") {
                    return trans_choice('savings::general.approved', 1);
                }
                if ($data->status == "declined") {
                    return trans_choice('savings::general.declined', 1);
                }
                return "N/A";
            })
            ->editColumn('action', function ($data) {

                $action = '<a href="' . url('savings/' . $data->savings_id . '/show') . '" class="btn btn-info">Saving ' . trans_choice('general.detail', 2) . '</a>';
                if($data->status == "pending" && (Auth::user()->roles->first()->name == "admin" || Auth::user()->roles->first()->name == "Loan Manager")){
                    $action .= '<button id="target" class="btn-round approval" style="margin-left:5px;"  data-id="'.$data->id.'" data-status="approved">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button id="target" class="btn-round approval" style="margin-left:5px;"  data-id="'.$data->id.'" data-status="declined">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    ...
                    <a href="'. url('savings/transaction/' . $data->id . '/pdf') . '"
                       target="_blank" class="dropdown-item"><i
                                class="fas fa-file-pdf"></i> Receipt
                    </a>

                    <a href="'. url('savings/transaction/' . $data->id . '/print') . '"
                       target="_blank" class="dropdown-item"><i
    ...
    public function addSavingDeposit()
    {
        ...
        $clients = Client::all();
        $payment_types = PaymentType::where('active', 1)->get();
        return theme_view('savings::deposit.add_deposit', compact('client_groups','clients','payment_types'));
    }
    ...
    public function getSavingsList(Request $request)
    {
        if ($request->ajax()) {
            $client_id = $request->client_id;
            //$savingsList = Savings::where('client_id',$client_id)
            $savingsList = Savings::with('transactions')->with('charges')->with('client')->with('savings_product')->where('client_id',$client_id)->get();
            return json_encode($savingsList);

        }
    }

    public function getSavingsDetail(Request $request)
    {
        if ($request->ajax()) {
            $saving_id = $request->saving_id;
            //$savingsList = Savings::where('client_id',$client_id)
            $savings = Savings::with('transactions')->with('charges')->with('client')->with('savings_product')->find($saving_id);
            return json_encode($savings);

        }
    }

    public function approve_reject_savings_deposit(Request $request)
    {

        $request->validate([
            'approved_on_date' => ['id','status'],
        ]);
        $savingsTransactions = SavingsTransaction::find($request->id);
        $savingsTransactions->checked_by = Auth::id();
        $savingsTransactions->status = $request->status;
        $savingsTransactions->remarks = $request->remarks;
        $savingsTransactions->save();
        \flash(trans_choice("core::general.successfully_saved", 1))->success()->important();
        return redirect('savings/deposit_report');
    }
