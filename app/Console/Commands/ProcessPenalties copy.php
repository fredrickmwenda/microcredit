...
class ProcessPenalties extends Command
{
 ...
 // ================================
        // 1️⃣ PROCESS MISSED PAYMENT PENALTY
        // ================================
        
        // Get today's date
        $due_date = Carbon::today()->format("Y-m-d");

        // Fetch overdue loan schedules where penalty not yet applied
        foreach (
            DB::table("loan_schedules")
                ...
                ->selectRaw(DB::raw("
                    loans.branch_id,
                    loans.principal original_principal,
                    loan_products.accounting_rule,
                    loan_products.late_repayment_penalty_grace_period,
                    loan_products.after_maturity_date_penalty_grace_period,
                    loan_schedules.*,
                    -- Summed overdue amounts before today
                    (SELECT SUM(principal) FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_principal,
                    (SELECT SUM(interest)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_interest,
                    (SELECT SUM(fees)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_fees,
                    (SELECT SUM(penalty)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_penalty,
                    -- Waived amounts
                    (SELECT SUM(principal_waived)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_principal_waived,
                    (SELECT SUM(interest_waived)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_interest_waived,
                    (SELECT SUM(fees_waived) FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_fees_waived,
                    (SELECT SUM(penalty_waived)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_penalty_waived,
                ...
                ->where('loan_schedules.missed_penalty_applied', 0)
                ->where('loans.status', 'disbursed')
                ->get()
            as $schedule
        ) {
            // Calculate outstanding balance for this schedule
            $schedule_balance =
                ($schedule->principal - $schedule->principal_waived +
                 $schedule->interest - $schedule->interest_waived +
                 $schedule->fees - $schedule->fees_waived +
                 $schedule->penalty - $schedule->penalty_waived)
                - ($schedule->principal_paid + $schedule->interest_paid + $schedule->penalty_paid + $schedule->fees_paid);

            // If still unpaid AND today is exactly grace_period days after due date
            if ($schedule_balance > 0 &&
                Carbon::today()->equalTo(Carbon::parse($schedule->due_date)->addDays($schedule->late_repayment_penalty_grace_period))
            ) {
                $principal_due = 0;
                $interest_due = 0;
                $total_due = 0;
                ...
                // Recalculate total amounts overdue before today for this loan
                foreach (
                    DB::table("loan_schedules")
                        ->join("loans", "loans.id", "loan_schedules.loan_id")
                        ->join("loan_products", "loan_products.id", "loans.loan_product_id")
                        ->selectRaw(DB::raw("
                            ...
                            loan_schedules.*,
                            (SELECT SUM(principal) FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_principal,
                            (SELECT SUM(principal_paid) FROM loan_schedules) total_principal_paid,
                            (SELECT SUM(interest)  FROM loan_schedules WHERE loan_schedules.due_date<$due_date AND loan_id=loans.id) total_interest,
                            (SELECT SUM(interest_paid)  FROM loan_schedules) total_interest_paid,
                            (SELECT SUM(fees)  FROM loan_schedules) total_fees,
                            (SELECT SUM(fees_paid)  FROM loan_schedules) total_fees_paid,
                            (SELECT SUM(penalty)  FROM loan_schedules) total_penalty,
                            (SELECT SUM(penalty_paid)  FROM loan_schedules) total_penalty_paid,
                        ...
                        "))
                        ->where('loan_schedules.loan_id', $schedule->loan_id)
                        ->where('loan_schedules.due_date', '<', $due_date)
                        ->where('loans.status', 'disbursed')
                        ->get()
                    as $key
                    ...
                    // Increment totals
                    $principal_due += ($key->principal - $key->principal_waived - $key->principal_paid);
                    $interest_due  += ($key->interest - $key->interest_waived - $key->interest_paid);
                    $total_due     += $principal_due + $interest_due +
                        ($key->fees - $key->fees_waived - $key->fees_paid) +
                        ($key->penalty - $key->penalty_waived - $key->penalty_paid);
                }

                // Fetch all overdue installment charges for this loan
                foreach (
                    LoanCharge::join('charges', 'charges.id', 'loan_charges.charge_id')
                        ->selectRaw("loan_charges.amount charge_amount, loan_charges.grace_period, charges.charge_option, charges.id")
                        ->where('charges.charge_type', 'overdue_installment_fee')
                        ->where('loan_charges.loan_id', $schedule->loan_id)
                        ->get()
                    as $key
                ) {
                    $amount = 0;

                    // Calculate penalty based on charge option
                    if ($key->charge_option == 'fixed') {
                        $amount = $key->charge_amount;
                    } elseif ($key->charge_option == 'principal_due') {
                        $amount = $key->charge_amount * $principal_due / 100;
                    } elseif ($key->charge_option == 'interest_due') {
                        $amount = $key->charge_amount * $interest_due / 100;
                    } elseif ($key->charge_option == 'total_due') {
                        $amount = $key->charge_amount * $total_due / 100;
                    } elseif ($key->charge_option == 'original_due') {
                        $amount = $key->charge_amount * $original_principal / 100;
                    }

                    // Update penalty amount in loan schedule
                    $adjusted_schedule = LoanSchedule::find($schedule->id);
                    $adjusted_schedule->penalty += $amount;
                    $adjusted_schedule->save();

                    // Record a penalty transaction
        ...
        // ================================
        // 2️⃣ PROCESS AFTER-MATURITY PENALTY
        // ================================
    ...
    /**
 * Process overdue maturity penalties for disbursed loans past maturity date
 */
protected function processOverdueMaturityPenalties($due_date)
{
    // Fetch all disbursed loans that have passed their maturity date
    $loans = DB::table("loans")
        ...
        ->selectRaw(DB::raw("
            ...
            loan_products.after_maturity_date_penalty_grace_period,
            loan_schedules.*,
            (SELECT SUM(principal) FROM loan_schedules WHERE loan_id=loans.id) total_principal,
            (SELECT SUM(principal_paid) FROM loan_schedules WHERE loan_id=loans.id) total_principal_paid,
            (SELECT SUM(interest) FROM loan_schedules WHERE loan_id=loans.id) total_interest,
            (SELECT SUM(interest_paid) FROM loan_schedules WHERE loan_id=loans.id) total_interest_paid,
            (SELECT SUM(fees) FROM loan_schedules WHERE loan_id=loans.id) total_fees,
            (SELECT SUM(fees_paid) FROM loan_schedules WHERE loan_id=loans.id) total_fees_paid,
            (SELECT SUM(penalty) FROM loan_schedules WHERE loan_id=loans.id) total_penalty,
            (SELECT SUM(penalty_paid) FROM loan_schedules WHERE loan_id=loans.id) total_penalty_paid,
            (SELECT SUM(principal_waived) FROM loan_schedules WHERE loan_id=loans.id) total_principal_waived,
            (SELECT SUM(interest_waived) FROM loan_schedules WHERE loan_id=loans.id) total_interest_waived,
            (SELECT SUM(fees_waived) FROM loan_schedules WHERE loan_id=loans.id) total_fees_waived,
            (SELECT SUM(penalty_waived) FROM loan_schedules WHERE loan_id=loans.id) total_penalty_waived,
            (SELECT SUM(credit) FROM loan_transactions 
                WHERE transaction_type='repayment' 
                AND reversed=0 
                AND loan_transactions.loan_id=loan_schedules.loan_id
            ) payments
        "))
        ->where('loans.status', 'disbursed')
        ->where('loans.maturity_date', '<', $due_date)
        ->groupBy('loans.id')
        ->get();
    ...
    foreach ($loans as $loan) {
        ...
        $balance = (
            $loan->principal - $loan->principal_waived +
            $loan->interest - $loan->interest_waived +
            $loan->fees - $loan->fees_waived +
            $loan->penalty - $loan->penalty_waived
        ) - (
            $loan->principal_paid +
            $loan->interest_paid +
            $loan->penalty_paid +
        ...
        // Only process if there is a balance AND today is the penalty application date
        if (
            $balance > 0 &&
            Carbon::today()->equalTo(
                Carbon::parse($loan->maturity_date)->addDays($loan->after_maturity_date_penalty_grace_period)
            )
        ) {
            // Amounts due
            $principal_due = $loan->total_principal - $loan->total_principal_waived - $loan->total_principal_paid;
            $interest_due = $loan->total_interest - $loan->total_interest_waived - $loan->total_interest_paid;
            $total_due = $principal_due + $interest_due +
                ($loan->total_fees - $loan->total_fees_waived - $loan->total_fees_paid) +
                ($loan->total_penalty - $loan->total_penalty_waived - $loan->total_penalty_paid);

            $original_principal = $loan->original_principal;

            // Get all overdue maturity charges for this loan
            $charges = LoanCharge::join('charges', 'charges.id', 'loan_charges.charge_id')
                ->selectRaw("loan_charges.amount charge_amount, loan_charges.grace_period, charges.charge_option, charges.id")
                ->where('charges.charge_type', 'overdue_maturity')
                ->where('loan_charges.loan_id', $loan->loan_id)
                ->get();

            foreach ($charges as $charge) {
                // Determine penalty amount based on charge option
                $amount = match ($charge->charge_option) {
                    'fixed'         => $charge->charge_amount,
                    'principal_due' => $charge->charge_amount * $principal_due / 100,
                    'interest_due'  => $charge->charge_amount * $interest_due / 100,
                    'total_due'     => $charge->charge_amount * $total_due / 100,
                    'original_due'  => $charge->charge_amount * $original_principal / 100,
                    default         => 0,
                };

                // Adjust last schedule with penalty
                $adjusted_schedule = LoanSchedule::where('loan_id', $loan->loan_id)->latest()->first();
                $adjusted_schedule->penalty += $amount;
                $adjusted_schedule->save();
                ...
                $loan_transaction->year = $date_parts[0];
                $loan_transaction->month = $date_parts[1];
                $loan_transaction->debit = $amount;
                $loan_transaction->save();
            }
        }
